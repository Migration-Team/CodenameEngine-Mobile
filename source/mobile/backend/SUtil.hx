...
class SUtil
{
	...
	public static function doTheCheck()
	{
		...
		if (Permissions.getGrantedPermissions().contains(PermissionsList.READ_EXTERNAL_STORAGE) || Permissions.getGrantedPermissions().contains(PermissionsList.WRITE_EXTERNAL_STORAGE))
		{
			if (!FileSystem.exists(Tools.getExternalStorageDirectory() + '/' + '.CodeNameEngine'))
				FileSystem.createDirectory(Tools.getExternalStorageDirectory() + '/' + '.CodeNameEngine');

			if (!FileSystem.exists(SUtil.getPath() + 'assets') && !FileSystem.exists(SUtil.getPath() + 'mods'))
			{
				SUtil.applicationAlert('Uncaught Error :(!', "Whoops, seems you didn't extract the files to the .CodeNameEngine!\nPlease watch the tutorial by pressing OK.");
			...
			else
			{
				...
				{
					SUtil.applicationAlert('Uncaught Error :(!', "Whoops, seems you didn't extract the assets folder to the .CodeNameEngine!\nPlease watch the tutorial by pressing OK.");
		            //CoolUtil.browserLoad('https://youtu.be/AmoNoYjJgHs?si=LvgXbRRn7eJlwL0w');
					System.exit(0);
				}
				...
				{
					SUtil.applicationAlert('Uncaught Error :(!', "Whoops, seems you didn't extract the mods folder to the .CodeNameEngine\nPlease watch the tutorial by pressing OK.");
		            //CoolUtil.browserLoad('https://youtu.be/AmoNoYjJgHs?si=LvgXbRRn7eJlwL0w');
					System.exit(0);
				}
			}
